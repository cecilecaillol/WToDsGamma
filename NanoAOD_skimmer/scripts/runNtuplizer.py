...
from WToDsGamma.NanoAOD_skimmer.EraConfig import *
...
def buildCondorFile(opt,FarmDirectory):
    ...
    with open (condorFile,'w') as condor:
        ...
        for dataset in datasets:
          ...
          if 'NanoAODv11' in dataset:
            ...
            sys.exit(1)
	  if "Muon" in dataset or "EGamma" in dataset:
            sufix='data'
	  else:
            sufix='mc'
    ...
    with open(workerFile,'w') as worker:
        ...
        worker.write('echo "--bi $CMSSW_BASE/src/WToDsGamma/NanoAOD_skimmer/scripts/keep_in.txt   \\\\"\n')
        worker.write('echo "--bo $CMSSW_BASE/src/WToDsGamma/NanoAOD_skimmer/scripts/keep_out.txt  \\\\"\n')
        worker.write('echo "${filter} -I WToDsGamma.NanoAOD_skimmer.WToDsGamma_analysis ${channel} "\n')
        worker.write('python $CMSSW_BASE/src/PhysicsTools/NanoAODTools/scripts/nano_postproc.py \\\n')
        worker.write('$filename ${input}  \\\n')
        worker.write('--bi $CMSSW_BASE/src/WToDsGamma/NanoAOD_skimmer/scripts/keep_in.txt   \\\n')
        worker.write('--bo $CMSSW_BASE/src/WToDsGamma/NanoAOD_skimmer/scripts/keep_out.txt  \\\n')
        worker.write('${filter} -I WToDsGamma.NanoAOD_skimmer.WToDsGamma_analysis ${channel} \n')
...
def main():
    ...
    parser.add_option('-o', '--out',      dest='output',   help='output directory',  default='/eos/cms/store/cmst3/user/ccaillol/WToDsGamma/ntuples_w_2018', type='string')
